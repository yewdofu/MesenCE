...
SNESコントローラーの押下状態を設定します。指定したボタンだけが押された状態になり、空配列を指定すると入力オーバーライドを解除します。実行中・停止中のどちらでも使用できます。

- パラメータ:

```json
{"jsonrpc":"2.0","id":2,"method":"input.set","params":{"controller":1,"buttons":["A","Right"]}}
```

| フィールド | 型 | 説明 |
|-----------|----|------|
| `controller` | int? | コントローラー番号（1〜8、既定1） |
| `buttons` | string[] | 押下するボタン。`A` / `B` / `X` / `Y` / `L` / `R` / `Up` / `Down` / `Left` / `Right` / `Select` / `Start`（大文字小文字は問わない） |

- 応答:

```json
{"jsonrpc":"2.0","id":2,"result":{"controller":1,"buttons":["A","Right"]}}
```

- 接続されていないコントローラー番号、未対応ボタン、範囲外の番号は`-32602`になります。
- 入力状態は次の`input.set`または`input.clear`まで維持されます。APIクライアントの切断時とAPIサーバー終了時には全コントローラーの入力を自動解除します。
- エラー: `-32001`（ROMなし）、`-32005`（SNES以外）、`-32602`

---

### input.clear

指定したSNESコントローラーの入力オーバーライドを解除します。

- パラメータ:

```json
{"jsonrpc":"2.0","id":3,"method":"input.clear","params":{"controller":1}}
```
...
```json
{"jsonrpc":"2.0","id":3,"result":{"controller":1,"buttons":[]}}
```
